...
- [x] G1: cron snapshot emits reminder text + schedule for reminder-shaped jobs
  ...
  EXPECT: G1 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G1 PASS: 21 reminders via the real script (--emit), text+note+schedule present, non-reminders clean
...
- [x] G2: drainer plans correct CLI ops for create/update/cancel from a queue fixture
  ...
  EXPECT: G2 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G2 PASS: create/update/snooze/cancel plan correct argv; payload is argv, not shell
...
- [x] G3: drainer is idempotent and rejects malformed input (bad cron expr, bad target, oversize text)
  ...
  EXPECT: G3 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G3 PASS: 13 malformed actions rejected, valid control accepted, applied ids skipped
...
- [x] G4: every inline script block in index.html still parses
  ...
  EXPECT: G4 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=(node:16798) ExperimentalWarning: VM Modules is an experimental feature and might change at any time | (Use `node --trace-warnings ...` to show where the warning was created)
...
- [x] G5: headless browser renders the Reminders page from a stubbed snapshot — rows, groups, and the create form are present
  ...
  EXPECT: G5 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G5 PASS: reminders render + group + humanize, non-reminders excluded, create queues a valid action; mobile drawer entry navigates (screenshots: /tmp/g5_reminders.png, /tmp/g5_reminders_mobile.png)
...
- [x] G7: Web Push payload encryption and VAPID signing are correct (round-trip, not assumed)
  ...
  EXPECT: G7 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G7 PASS: aes128gcm round-trips, salt/keys fresh per send, VAPID JWT verifies against its advertised key, tampering rejected
...
- [x] G8: a fired reminder hits Discord and push, and neither channel can suppress the other
  ...
  EXPECT: G8 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G8 PASS: both channels fire; push failure never costs the reminder; discord failure still reports
...
- [x] G9: the service worker renders every push shape and a tap reuses the open dashboard
  ...
  EXPECT: G9 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/reminders; path=2e1f78c509a3/13 entries; output=G9 PASS: push renders (JSON, text and empty payloads); tap dismisses, focuses and navigates the open dashboard
