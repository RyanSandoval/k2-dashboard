...
- [x] G12: `t` on Stale Jots starts a full-keyboard triage pass instead of navigating to Tasks, and one keypress does exactly one thing
  CHECK: python3 /Users/ryansandoval/k2-dashboard/spec/ui-rebuild/checks/g12_keyboard_triage.py
  EXPECT: G12 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/ui-rebuild; output=G12 PASS: `t` on Stale Jots enters triage without the global `t` also firing; j/k move a marked cursor over 30 rows and scroll it into view without reaching global `j`; t/n/s/x each mutate DATA (task created, note body appended with no modal, archive and discard recorded, discarded line removed from the daily doc) and the counter falls 30->26; four undos restore every one of those plus the cursor; stubbing the undo stack, the cursor attribute and staleJotsTriageStart() each control-fail; Escape exits and global d/j/t work again; 390px cannot enter. Control test: with index.html reverted the gate fails 27 assertions, restored it passes. Surprises: (1) the mode had to claim every unmodified key while it is on, not just its six verbs — swallowing only j/k/t/n/s/x/u leaves `d` navigating to the Dashboard with triage still active and a cursor pointing at a list nobody is looking at; (2) the cursor mark and the counter both had to be written inside renderStaleJots() rather than after it, because the gate's control test wraps renderStaleJots and strips the attribute on the way out — anything painted afterwards would have survived stripping and proved nothing; (3) undo needed no per-verb restore shape: note bodies and daily-doc contents are immutable strings, so remembering the object reference plus its old value covers all four verbs in one snapshot; (4) `t` cannot double as the exit, because `t` is the convert-to-task verb inside the mode — Escape is the only exit, routed above the typing guard where 51a2b94 put Escape.

- [x] G13: a task sequence that repeats a third time raises exactly one save-as-template suggestion, and a saved template spawns the whole checklist in one tap
  CHECK: python3 /Users/ryansandoval/k2-dashboard/spec/ui-rebuild/checks/g13_task_templates.py
  EXPECT: G13 PASS
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/ryansandoval/k2-dashboard/spec/ui-rebuild; output=G13 PASS: 4 steps repeated twice raise nothing and a third repeat raises exactly one suggestion naming both counts, with 610 sourcePlanFile rows in the same store contributing zero; saving writes one DATA.taskTemplates row and retires the suggestion; one tap on the chip pushes 4 open tasks onto proj-x with makeTask()'s full shape and 4 distinct ids, and the board renders them; dismiss suppresses by signature without leaving a usable chip. Controls: dropping a run, marking the seed sourcePlanFile, restoring Date.now() ids and emptying the store each fail their assertion. Control test: with index.html reverted the gate fails on "detectTaskTemplates() is not defined - the feature is absent"; restored it passes. Note: the ~610 plan-sync duplicate rows are excluded by field (sourcePlanFile), not by heuristic - they otherwise read as a genuine repeating sequence and would have been the loudest false template in the store.
